...
<form style="justify-content: center; padding: 20px; display: flex" action="{{ route('projects.update', $project['id']) }}" method="POST">
        @method('PUT') @csrf
    <label style="padding: 12px 12px 12px 0; display: inline-block;" for="name">Project name:</label>
    <input style="padding: 12px; border: 1px solid #ccc; border-radius: 4px; font-size: medium;" type="text" id="name" name="project_name" value="{{ $project->project_name}}" placeholder="Project name">
...
<label style="justify-content: center; padding: 20px; display: flex;">Project employees:
    ...
    <div style="position: relative; margin-left: 12px; padding: 0px 30px 0px 20px; border: 1px solid lightgrey; border-radius: 5px;"> {{$employee->employee_name}}
        <a style= "position: absolute; right: 0;padding: 0px; padding-right: 3px;line-height: 9px;color: red; font-size: small;" href="{{ route('projects.deleteEmployee', ['projectId' => $project->id, 'employeeId' => $employee->id]) }}">
